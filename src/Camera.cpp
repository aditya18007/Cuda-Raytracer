...
: m_width(width), m_height(height), lastTime(glfwGetTime()), camera_state(Camera_State::STILL), m_camPos(0,0,5)
{
	mouse_x = width/2;
	mouse_y = height/2;
}
...
void Camera::update_mouse_pos(float x, float y) {
	mouse_x = x;
	mouse_y = y;
	camera_state = Camera_State::UPDATE_NEEDED;
}

void Camera::update() {
	
	if ( camera_state == Camera_State::STILL){
		return;
	}
	...
	camera_state = Camera_State::STILL;
}
